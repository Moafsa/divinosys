...
(
  echo "*/2 * * * * curl -s http://localhost/mvc/ajax/process_payment_reminders.php > /dev/null 2>&1"
  echo "*/5 * * * * curl -s http://localhost/mvc/ajax/process_ai_automations.php > /dev/null 2>&1"
) | crontab -
service cron start
